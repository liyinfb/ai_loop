#!/usr/bin/env python3
"""
test_loop_v3.py — 24 test functions for loop_v3.py
==============================

Tests cover:
  - Finding dataclass (serialize/round-trip)
  - Memory (save, load, update, archive, state, clear)
  - EvaluatorAgent (has_structure, evaluate, add_rule, all_pass)
  - GeneratorAgent (create_worktree, generate, cost tracking)
  - BudgetGate (all three gates)
  - LoopOrchestrator (full run_turn cycle)
  - Human gate
  - V3-specific differences (add_rule two-arg signature, no connectors)
"""
...
# 9. Memory — load_findings ignores connectors (V3 doesn't use them)
def test_memory_load_findings_ignores_connectors():
    mem = _make_memory()
    f1 = _make_finding(status=Status.PENDING)
    f2 = _make_finding(status=Status.PENDING)
    mem.save_finding(f1)
    mem.save_finding(f2)
    # Pass a dummy list
    result = mem.load_findings(connectors=[1, 2, 3])
    assert len(result) >= 0  # V3 returns what's in _findings regardless of connectors
    print("  [PASS] test_memory_load_findings_ignores_connectors")


# 10. Memory — clear removes in-memory state
def test_memory_clear():
    mem = _make_memory()
    mem.save_finding(_make_finding())
    mem.save_finding(_make_finding(id="t2"))
    mem.clear()
    assert len(mem._findings) == 0
    assert len(mem._findings_index) == 0
    print("  [PASS] test_memory_clear")


# 11. EvaluatorAgent — has_structure detects headings
...
# 12. EvaluatorAgent — evaluate with no rules returns empty
...
# 13. EvaluatorAgent — all_pass with empty list returns True
...
# 14. EvaluatorAgent — add_rule (two-arg signature) and evaluate passes
def test_evaluator_custom_rules_pass():
    ev = EvaluatorAgent()
    # V3: two-arg signature (draft, worktree)
    ev.add_rule("has_content", lambda d, w: {"passed": len(d) > 50, "detail": "ok"})
    f = _make_finding()
    results = ev.evaluate(f, "# Title\n" + "x" * 60, "/tmp/wt")
...
# 15. EvaluatorAgent — add_rule (two-arg signature) and evaluate fails content check
...
# 16. EvaluatorAgent — error in rule handler returns failed verdict
def test_evaluator_rule_crashes():
    ev = EvaluatorAgent()
    ev.add_rule("crash_rule", lambda d, w: 1/0)
    f = _make_finding()
    results = ev.evaluate(f, "# Title\nContent\n", "/tmp/wt")
    assert len(results) == 1
    assert results[0]["passed"] == False
    assert "crash" in results[0]["detail"].lower()
    print("  [PASS] test_evaluator_rule_crashes")
...
# 18. GeneratorAgent — create_worktree creates directory
...
# 20. GeneratorAgent — cost tracking works
def test_generator_cost_tracking():
    root = tempfile.mkdtemp()
    gen = GeneratorAgent(worktrees_root=root)
    gen.track_cost(5.0)
    gen.track_cost(3.0)
    assert gen.current_cost == 8.0
    gen.reset_budget()
    assert gen.current_cost == 0.0
    shutil.rmtree(root)
    print("  [PASS] test_generator_cost_tracking")


# 21. BudgetGate — pre_discovery gate rejects over budget
...
# 22. LoopOrchestrator — run_turn with no findings returns False
...
# 23. LoopOrchestrator —— full round-trip with one finding (simulate all-pass)
def test_orchestrator_full_roundtrip():
    ...
    ev = EvaluatorAgent()
    # V3: two-arg signature
    ev.add_rule("always_pass", lambda d, w: {"passed": True, "detail": "ok"})
...
# 24. LoopOrchestrator — human gate blocks approval
def test_orchestrator_human_gate_blocks():
    mem = _make_memory()
    f = _make_finding(id="gate-test", status=Status.NEEDS_REVIEW)
    mem.save_finding(f)
    ...
    with patch("builtins.input", return_value="n"):
        result = orch.run_turn("template text")
        assert result == True
        # Finding should still be in inbox (not archived) since human said "n"
        still_pending = [fi for fi in mem.get_all_findings() if fi.id == "gate-test"]
        # The finding went through but status should be NEEDS_REVIEW not ARCHIVED
        print("  [PASS] test_orchestrator_human_gate_blocks")
...
def run_all_tests():
    ...
    tests = [
        ...
        test_memory_load_findings_ignores_connectors,
        test_memory_clear,
